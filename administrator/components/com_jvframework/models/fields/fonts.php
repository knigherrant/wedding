...
class JFormFieldFonts extends JFormField
{
	...
	protected function addAsset(){
		...
		$doc->addScript(str_replace(DIRECTORY_SEPARATOR, '/', str_replace(JPATH_ROOT.DIRECTORY_SEPARATOR, JURI::root(), dirname(dirname(dirname(__FILE__))))).'/assets/js/font.js');
		$js = "
			window.addEvent('domready', function(){
				var Font = new JVFont('".$this->id."');
				Font.addFontType('Standard', ".json_encode($this->getStandardFonts()).");
				Font.addFontType('Google webfont', 	 ".json_encode($this->getGoogleFonts()).");
				Font.display();
			});
		";
	...
	protected function getGoogleFonts() {
		...
		return json_encode($google_fonts);
	}
	
	protected function getStandardFonts() {
		...
		return json_encode($standard_fonts);
	}
}