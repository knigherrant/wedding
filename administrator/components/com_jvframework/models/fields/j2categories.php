...
class JFormFieldJ2categories extends JFormField {
	...
	private function _getHTML(){
		...
		$customfields = $this->_getField();
                if(version_compare(JVERSION, '3', '>=')) JHtml::_('formbehavior.chosen', 'select');
		$html .= '
	...
	private function _getField(){
		...
		$customfields = $this->form->getValue('customfield','params');
                ...
                $output= JHTML::_('select.genericlist',  $mitems, $this->name, 'class="inputbox" multiple="multiple"', 'value', 'text', $this->value, $this->id );
                return $output;	
	}
}
