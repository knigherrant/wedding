 ...
 * @since       11.1
 */
class JInstallerTheme extends JAdapterInstance
{
	/**
	...
	public function loadLanguage($path = null)
	{
		...
		$basePath = end(JV::_('path.getPath', 'themes'));
		...
		if ($this->manifest->files)
		{
			...
			if (count($element->children()))
			{
				...
				{
					if ((string) $file->attributes()->theme)
					{
						$extension = strtolower((string) $file->attributes()->theme);
						break;
					}
				}
			}
	...
	public function install()
	{
		...
		if (count($this->manifest->files->children()))
		{
			foreach ($this->manifest->files->children() as $file)
			{
				if ((string) $file->attributes()->theme)
				{
					$element = (string) $file->attributes()->theme;
		...
		$query->where($query->qn('element') . ' = ' . $query->q($element))->where($query->qn('type') . ' = ' . $query->q('theme'))->where($query->qn('client_id') . ' = ' . (int) $clientId);		
		$db->setQuery($query);
		...
		else
		{
			...
			$this->parent->pushStep(array('type' => 'theme', 'extension_id' => $row->extension_id));

			// Create unpublished module in jos_modules
			$name = $this->get('element');
			
			$db = JFactory::getDbo();
			$db->setQuery("SELECT id FROM #__jv_themes WHERE theme='{$name}'");
			if (!$db->loadResult()){				
				// 
				$theme = JTable::getInstance('theme', 'Table');
				$theme->set('theme', $name);
				$theme->set('published', 1);
				$theme->store();
				
				// Create first style
				$style = JTable::getInstance('style', 'Table');
				$style->set('theme', $name);
				$style->set('title', $row->name);
				$style->set('home', 0);
				$style->store();
			}
		}
	...
	public function discover()
	{
		...
		{
			$manifest_details = JApplicationHelper::parseXMLInstallFile($basePath."/$extension/themeInfo.xml");
	...
	public function discover_install()
	{
		...
		$basePath = end(JV::_('path.getPath', 'themes'));
		$manifestPath = $basePath . '/' . $this->parent->extension->element . '/themeInfo.xml';
	...
	public function uninstall($id)
	{
		...
		$db->setQuery("SELECT id FROM #__jv_theme_styles WHERE theme='{$row->element}'");
		if($db->loadResult()){
			JError::raiseWarning(100, JText::_('Cannot uninstall "'.$row->element.'", please remove all style(s) related to this theme before uninstall!'));
			return false;
		}
		...
		// Remove theme		
		$query = $db->getQuery(true);
		$query->delete()->from('#__jv_themes')->where('theme = ' . $query->quote($row->element) );
		$db->setQuery($query);
		$db->execute();
