 ...
 * @since       11.1
 */
class JVInstallerExtension extends JAdapterInstance
{
	/**
	...
	public function loadLanguage($path = null)
	{
		...
		$jPath = JV::_('path.getPath', 'extensions');
		$basePath = end($jPath);
		...
		if (!$source)
		{
			$this->parent
				->setPath(
				'source',
				$basePath. $this->parent->extension->element
			);
		}
		...
		if ($this->manifest->files)
		{
			...
			if (count($element->children()))
			{
				...
				{
					if ((string) $file->attributes()->extension)
					{
						$extension = strtolower((string) $file->attributes()->extension);
						break;
					}
				}
			}
			...
			if ($extension)
			{
				...
				$source = $path ? $path : ($basePath . $extension);
	...
	public function install()
	{
		...
		if (count($this->manifest->files->children()))
		{
			foreach ($this->manifest->files->children() as $file)
			{
				if ((string) $file->attributes()->extension)
				{
					$element = (string) $file->attributes()->extension;
		...
		if (!empty($element))
		{
			$this->parent->setPath('extension_root', $basePath . '/' . $element);
		}
		else
		{
		...
		$query->where($query->qn('element') . ' = ' . $query->q($element))->where($query->qn('type') . ' = ' . $query->q('extension'))->where($query->qn('client_id') . ' = ' . (int) $clientId);		
		$db->setQuery($query);
		...
		// If the extension directory already exists, then we will assume that the
		// extension is already installed or another extension is using that
		// directory.
		...
		// Was there a extension already installed with the same name?
		if ($id)
		{
		...
		else
		{
			...
			$row->set('type', 'extension');
			$row->set('element', $this->get('element'));
			$row->set('folder', ''); // There is no folder for extensions
			$row->set('enabled', 1);
			$row->set('protected', 0);
	...
	{
		$results = array();
		$basePath = end(JV::_('path.getPath', 'extensions'));
		$list = JFolder::folders($basePath);
		$site_info = JApplicationHelper::getClientInfo('site', true);
	
		foreach ($list as $extension)
		{
			$manifest_details = JApplicationHelper::parseXMLInstallFile($basePath."/$extension/config.xml");
			$extension = JTable::getInstance('extension');
			$extension->set('type', 'extension');
			$extension->set('client_id', $site_info->id);
			$extension->set('element', $extension);
	...
	public function discover_install()
	{
		...
		$basePath = end(JV::_('path.getPath', 'extensions'));
		$manifestPath = $basePath . '/' . $this->parent->extension->element . '/config.xml';
		...
		{
            JFactory::getApplication()->enqueueMessage(JText::_('JLIB_INSTALLER_ERROR_MOD_DISCOVER_STORE_DETAILS'), 'warning');
	...
	public function refreshManifestCache()
	{
		...
		{
			JFactory::getApplication()->enqueueMessage(JText::_('JLIB_INSTALLER_ERROR_MOD_REFRESH_MANIFEST_CACHE'), 'warning');
			return false;
		}
	}
	 ...
	 *
	 * @param   integer  $id  The id of the extension to uninstall
	 *
	 * @return  boolean  True on success
	 *
	 * @since   11.1
	...
	public function uninstall($id)
	{
		...
		// First order of business will be to load the extension object table from the database.
		// This should give us the necessary information to proceed.
		$row = JTable::getInstance('extension');

		if (!$row->load((int) $id) || !strlen($row->element))
		{
            JFactory::getApplication()->enqueueMessage( JText::_('JLIB_INSTALLER_ERROR_MOD_UNINSTALL_ERRORUNKOWNEXTENSION'),'warning');
			return false;
		}

		// Is the extension we are trying to uninstall a core one?
		// Because that is not a good idea...
		if ($row->protected)
		{
			JFactory::getApplication()->enqueueMessage( JText::sprintf('JLIB_INSTALLER_ERROR_MOD_UNINSTALL_WARNCOREMODULE', $row->name),'warning');
			return false;
		}
		...
		$this->loadLanguage($basePath . $element);
		...
		if (!($this->manifest instanceof SimpleXMLElement))
		{
            ...
            JFactory::getApplication()->enqueueMessage( JText::_('JLIB_INSTALLER_ERROR_MOD_UNINSTALL_INVALID_NOTFOUND_MANIFEST'),'warning');
		...
		{
			// Install failed, rollback changes
            JFactory::getApplication()->enqueueMessage( JText::sprintf('JLIB_INSTALLER_ERROR_MOD_UNINSTALL_SQL_ERROR',  $db->stderr(true)),'warning');
	 ...
	 * @since   11.1
	 */
	protected function _rollback_extension($arg)
	{
	}
}
