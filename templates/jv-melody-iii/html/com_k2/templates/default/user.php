...
?>
    <div class="boxDescription clearfix">
      <?php if ($this->params->get('userImage') && !empty($this->user->avatar)): ?>
      <img class="left" src="<?php echo $this->user->avatar; ?>" alt="<?php echo $this->user->name; ?>"  />
      <?php endif; ?>
      <div>
        <?php if($this->params->get('userFeedIcon',1)): ?>
        <!-- RSS feed icon -->
        <div class="k2FeedIcon"> <a href="<?php echo $this->feed; ?>" title="<?php echo JText::_('K2_SUBSCRIBE_TO_THIS_RSS_FEED'); ?>"> <span class="icon-rss"></span> </a> </div>
        <?php endif; ?>
  ...
  <div class="ItemList">
    ...
    <div class="ItemView<?php if(!$item->published || ($item->publish_up != $this->nullDate && $item->publish_up > $this->now) || ($item->publish_down != $this->nullDate && $item->publish_down < $this->now)) echo ' ItemViewUnpublished'; ?><?php echo ($item->featured) ? ' ItemIsFeatured' : ''; ?> clearfix"> 
      ...
      <div class="ItemBody">
        ...
        <a class="ItemImage" href="<?php echo $item->link; ?>" title="<?php if(!empty($item->image_caption)) echo K2HelperUtilities::cleanHtml($item->image_caption); else echo K2HelperUtilities::cleanHtml($item->title); ?>"> <img src="<?php echo $item->imageGeneric; ?>" alt="<?php if(!empty($item->image_caption)) echo K2HelperUtilities::cleanHtml($item->image_caption); else echo K2HelperUtilities::cleanHtml($item->title); ?>"  /> </a>
        <?php endif; ?>
        ...
        <div class="ItemLinks">
          ...
          <!-- Date created --> 
          <span class="ItemDateCreated"> <span class="icon-calendar"></span> <span class="month">
          <?php  echo JHTML::_('date',$item->created,'F');?>
          </span> <span class="day"> <?php echo JHTML::_('date',$item->created,'d'); ?> </span> <span class="years">
          ...
          <?php if($this->params->get('userItemCommentsAnchor') && ( ($this->params->get('comments') == '2' && !$this->user->guest) || ($this->params->get('comments') == '1')) ): ?>
              ...
              <?php if(!empty($this->event->K2CommentsCounter)): ?>
                  <!-- K2 Plugins: K2CommentsCounter --> 
                  <?php echo $this->event->K2CommentsCounter; ?>
                  <?php else: ?>
                  <?php if($item->numOfComments > 0): ?>
                        <a href="<?php echo $item->link; ?>#itemCommentsAnchor"> <span class="icon-comments"></span> <?php echo $item->numOfComments; ?> <?php echo ($item->numOfComments>1) ? JText::_('K2_COMMENTS') : JText::_('K2_COMMENT'); ?> </a>
                  <?php else: ?>
                  		<a href="<?php echo $this->link; ?>#itemCommentsAnchor"> <span class="icon-comments"></span> 0 <?php echo JText::_('K2_COMMENT'); ?> </a>
                  <?php endif; ?>
              <?php endif; ?>
              </span>
          <?php endif; ?>
          ...
          <!-- Item tags --> 
          <span class="ItemTags"> <span class="icon-tag"></span>
        ...
        <?php echo $item->event->K2AfterDisplayContent; ?> </div>
      <?php if ($this->params->get('userItemReadMore')): ?>
      <!-- Item "read more..." link -->
      <div class="ItemReadMore"> <a class="btn btn-default k2ReadMore" href="<?php echo $item->link; ?>"> <?php echo JText::_('K2_READ_MORE'); ?> </a> </div>
      <?php endif; ?>
